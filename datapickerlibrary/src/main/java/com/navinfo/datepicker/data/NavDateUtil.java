 ...
 * Describe
 */
public class NavDateUtil<T extends BaseNavDate> {
    ...
    private static ChineseDate calendarToChineseDate(Calendar calendar) {
        ...
        int index = year - DAYS_AND_LEAP_MONTH_OF_LUNAR[0];
        int data = (year << 9) | (month << 5) | date;
        ...
        long offset = chineseDateToNum(year, month, date) - chineseDateToNum(y, m, d);
        ...
        chineseDate.setYear(lunarY);
        chineseDate.setMonth(lunarM);
        chineseDate.setLeap(false);
        if (leap != 0 && lunarM > leap) {
            chineseDate.setMonth(lunarM - 1);
            if (lunarM == leap + 1) {
                chineseDate.setLeap(true);
            }
        }
        chineseDate.setDate(lunarD);
        return chineseDate;
    }

    private static long chineseDateToNum(int year, int month, int day) {
        month = (month + 9) % 12;
        year = year - month / 10;
